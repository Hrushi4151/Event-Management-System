...
import { HiUser, HiMail, HiPhone, HiPencil, HiSave, HiKey, HiTrash, HiCamera, HiTicket, HiCheckCircle, HiCalendar, HiStar, HiX, HiLockClosed, HiRefresh, HiEye, HiEyeOff } from 'react-icons/hi';
  ...
  useEffect(() => {
    const fetchProfile = async () => {
      ...
      try {
        ...
        const statsRes = await fetch(`/api/registration/user/${session.user.id}`, {
          cache: 'no-store',
        });
        if (statsRes.ok) {
          const registrations = await statsRes.json();
          const now = new Date();
          ...
          const checkedInCount = acceptedRegistrations.filter(reg => {
             if (reg.leader?.email === session?.user?.email) return reg.checkedIn;
             const member = reg.teamMembers?.find(m => m.email === session?.user?.email);
             return member?.attended;
          }).length;

          const attendanceRate = acceptedRegistrations.length > 0 
  ...
  return (
    <div className="min-h-screen bg-[#F8FAFC] py-8 px-4 md:px-8">
      ...
      <div className="max-w-6xl mx-auto mb-8">
        ...
        <p className="text-[#64748B] mt-2">Manage your account settings and preferences</p>
      </div>
        ...
        <div className="bg-white rounded-2xl border border-[#E5E7EB] p-8">
          <div className="grid grid-cols-1 md:grid-cols-4 gap-8">
            ...
            <div className="flex flex-col items-center gap-4">
              ...
              {profile.college && <p className="text-sm text-[#64748B] text-center">{profile.college}</p>}
            </div>
